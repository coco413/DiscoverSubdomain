...
class SubNameBrute:
    def __init__(self, target, threads=100, mode=False):
        ...
        self.resolvers = [
            dns.resolver.Resolver(
                configure=False) for _ in range(
                self.threads)]
    ...
    def _test_server(self, server):
        ...
        try:
            ...
            try:
                ...
                resolver.query('test.bad.dns.lijiejie.com')
            ...
            self._print_msg(
                '[+] Check DNS Server %s < OK >   Found %s' %
                (server.ljust(16), len(
                    self.dns_servers)))
        except:
            self._print_msg(
                '[+] Check DNS Server %s <Fail>   Found %s' %
                (server.ljust(16), len(
                    self.dns_servers)))

    def _load_sub_names(self):
        self._print_msg('[+] Load sub names ...')
        if self.full_scan and self.file == 'subnames.txt':
            _file = 'dict/subnames_full.txt'
        else:
            if os.path.exists(self.file):
                _file = self.file
            elif os.path.exists('dict/%s' % self.file):
                _file = 'dict/%s' % self.file
            else:
                self._print_msg(
                    '[ERROR] Names file not exists: %s' %
                    self.file)
                exit(-1)
    ...
    def put_item(self, item):
        num = item.count('{alphnum}') + \
            item.count('{alpha}') + item.count('{num}')
        if num == 0:
            self.priority += 1
            self.queue.put((self.priority, item))
        else:
        ...
        while not self.queue.empty():
            ...
            try:
                ...
                if answers:
                    ...
                    try:
                        self.resolvers[j].query(
                            'lijiejietest.' + cur_sub_domain)
    ...
    def run(self):
        ...
        try:
            ...
            return self.result_domains


if __name__ == '__main__':
    _ = SubNameBrute(target='baidu.com')
    _.run()
